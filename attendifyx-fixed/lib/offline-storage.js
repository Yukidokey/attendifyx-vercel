// IndexedDB is browser-only. All functions here are safe to import in
// server-side API routes — they return early/null when not in a browser.
...
export async function saveAttendanceOffline(attendanceData) {
  ...
  await addToSyncQueue({ type: 'attendance', action: 'create', data: record, status: 'pending' });
  return record;
}
...
export async function addToSyncQueue(item) {
  const db = await getOpenDB();
  if (!db) return null;
  return await db.add(STORES.SYNC_QUEUE, { ...item, created_at: new Date().toISOString() });
}
...
export async function updateSyncQueueItem(id, updates) {
  ...
  if (item) await db.put(STORES.SYNC_QUEUE, { ...item, ...updates });
}
...
export async function clearSyncedQueueItems() {
  ...
  for (const item of items) {
    if (item.status === 'completed') await db.delete(STORES.SYNC_QUEUE, item.id);
  }
}
...
export async function syncOfflineData(supabase) {
  ...
  try {
    ...
    return { success: true, message: `Synced ${syncedCount} items`, synced: syncedCount };
  } catch (error) {
